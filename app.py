    ...
    def vc_fn(input_audio, f0_up_key, f0_method, index_rate):
        try:
            ...
            audio_opt = vc.pipeline(
                hubert_model, net_g, 0, audio, [0, 0, 0], int(f0_up_key), f0_method,
                file_index, file_big_npy, index_rate, if_f0
            )
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M')}]: npy: 0s, f0: 0s, infer: 0s"
            )
        ...
        with gr.Tabs():
            for (name, title, cover, vc_fn) in models:
                with gr.TabItem(name):
                    ...
                    with gr.Row():
                        with gr.Column():
                            vc_input = gr.Textbox(label="Input audio path") if args.files else gr.Audio(label="Input audio")
